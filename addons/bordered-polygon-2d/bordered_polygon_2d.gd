...
func get_smooth_max_angle():
	...
	return deg_to_rad(smooth_max_angle)
...
func partition_id(angle, partition_size):
	return int(rad_to_deg(angle) / rad_to_deg(partition_size))
...
func create_border(border_size, quad, offset=Vector2(0,0)):
	...
	if has_border_textures():
		...
		tex.set_flags(tex.get_flags() | TEXTURE_REPEAT_ENABLED)
		border.set_texture(tex)

		var texture_rotation = deg_to_rad(border_texture_rotation) + PI
